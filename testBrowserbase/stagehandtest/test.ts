...
function validateJobs(jobs: Array<{
  company?: string;
  jobTitle?: string;
  location?: string;
  jobType?: string;
...
async function runWorkflow() {
  ...
  try {
    ...
    // Step 2: Extract data
    console.log("Extracting: extract all job listings from the first page including company name, job title, location, job type, and when it was posted");
    const extractedData2 = await page.extract({
      instruction: "extract all job listings from the first page including company name, job title, location, job type, and when it was posted",
      schema: z.object({
